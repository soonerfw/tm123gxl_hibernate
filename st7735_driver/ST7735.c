...
#define  TFT_CS_PIN     GPIO_PIN_3   // TFT_CS pin PA3
#define  CARD_CS_PIN    GPIO_PIN_4   // CARD_CS pin PA4
#define  DATA_CMD_PIN   GPIO_PIN_6   // Data/Command Pin PA6  =1  Data,  =0 Command
#define  RESET_PIN      GPIO_PIN_7   // Reset pin  PA7
...
void static writecommand(uint8_t c) {
 ...
 while(SSIBusy(SSI0_BASE))
  ...
  GPIOPinWrite(GPIO_PORTA_BASE, DATA_CMD_PIN,0);  // DATA_CMD_PIN = 0, command
...
void static writedata(uint8_t c) {
 while(SSIBusy(SSI0_BASE))
  ...
  GPIOPinWrite(GPIO_PORTA_BASE, DATA_CMD_PIN,DATA_CMD_PIN);  // DATA_CMD_PIN = 1, Data
	SSIDataPut(SSI0_BASE, c);
  while(SSIBusy(SSI0_BASE))
        {
        }			
...
//
  ...
  GPIOPinTypeGPIOOutput(GPIO_PORTA_BASE, TFT_CS_PIN|DATA_CMD_PIN|RESET_PIN); // TFT_CS, DATA/Command Pin and Reset pin all digital output
	
	// TFT_CS low
  GPIOPinWrite(GPIO_PORTA_BASE, TFT_CS_PIN,0);  // TFT_CS low
  GPIOPinWrite(GPIO_PORTA_BASE, RESET_PIN,RESET_PIN);  // RESET high

	Delay1ms(500);
  GPIOPinWrite(GPIO_PORTA_BASE, RESET_PIN,0);  // reset low
  ...
  // initialize SSI0, PA2 - SSI0CLK, PA5, SSIOTX,  PA3 SSIOFSS   if used for both SD card and ST7735 LCD, PA3 can only be used as GPIO, 
		

  SysCtlPeripheralEnable(SYSCTL_PERIPH_SSI0);  // enable SSI0
	
	GPIOPinConfigure(GPIO_PA2_SSI0CLK);
  GPIOPinConfigure(GPIO_PA3_SSI0FSS);  // configures PA2, PA3, PA5 as SSI0CLK, SSI0FSS, SSI0TX 
  ...
  GPIOPinTypeSSI(GPIO_PORTA_BASE, GPIO_PIN_2 | GPIO_PIN_3 | GPIO_PIN_5);		
    ...
    // CPOL = 0, CPHA = 0, mode 0, 8-bit SPI0, 5Mhz SPI clock frequency
//    SSIConfigSetExpClk(SSI2_BASE, g_ui32SysClock, SSI_FRF_MOTO_MODE_0, SSI_MODE_MASTER, 100000, 8);

  //SSIConfigSetExpClk(SSI0_BASE, g_ui32SysClock, SSI_FRF_MOTO_MODE_0, SSI_MODE_MASTER, 5000000,8);
...
// ST7734 LCD driver for top/bottom string display
// Input: device - 0, top;  non-zero: bottom
//        line   - 0-7,   line number in the top or bottom
//        string -  pointer null terminated ASCII string
//        value  -  # of chars to display
// Output: None
//         ST7735 LCD  16 rows, 21 columns 
void ST7735_Message (uint8_t device, uint8_t line, char *string, uint32_t value)
{
	  if(line>=5) {line=5;}
	  if(device==0)  // top 
		{
